...
LLM Exporter Local is intentionally local-only and user-triggered:
...
The extension reads only DOM content rendered in the active supported ChatGPT, Grok, Claude, or Gemini tab. It does not access hidden model reasoning, vendor APIs, account cookies, or server-side conversation data.

## Data handling

Captured conversation data exists in the active tab's injected script and the extension popup while it is open. It is not persisted by the extension. Closing the popup discards the in-memory capture unless the user downloaded the generated file.
...
4. Load the unpacked extension in a clean Chrome profile and test a synthetic or non-sensitive short, medium, and long conversation on each supported site.
